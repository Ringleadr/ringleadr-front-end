...
class NewStorage extends Component {
  ...
  handleSubmit() {
    ...
    if (this.state.value === '') {
      this.setState({showFailure: true, failureMessage: 'Storage name cannot be empty'});
      return
    }
  ...
  render() {
    return (
      <React.Fragment>
        ...
        <Message positive hidden={!this.state.showSuccess}>Storage successfully created</Message>
