...
const errorLabelNoPoint = <Label className={'custom-label'} color="red" horizontal/>;
const IPPortRegex = "^((([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]):)?[0-9]+$";
    ...
    return (
      <div className={'comp-wrapper'}>
        <div className={'comp-form'}>
          ...
          <Form.Field required>
            <label>Image</label>
            <Input name={'image'} value={this.props.component.image|| ''} placeholder={'Image'} onChange={this.props.propsOnChange} required
                   validationErrors={{isDefaultRequiredValue: 'Component name cannot be blank'}} errorLabel={errorLabel}/>
          </Form.Field>

          <Form.Field>
            <label>Replicas</label>
            <Input name={'replicas'} value={this.props.component.replicas|| ''} placeholder={'#Replicas'} onChange={this.props.propsOnChange}
                   instantValidation validations={'isValidInt'} validationError={'Replicas must be a positive Integer'} errorLabel={errorLabel}/>
          </Form.Field>
          ...
          <Form.Field>
            ...
            <Input name={'scale_threshold'} value={this.props.component.scale_threshold|| ''} placeholder={'Scale Threshold (100 equals 1CPU)'} onChange={this.props.propsOnChange}
                   instantValidation validations={'isValidFloat'} validationError={'Scale Threshold must be a float'} errorLabel={errorLabel}/>
          </Form.Field>
          ...
          <Form.Field>
            ...
            <Input name={'scale_min'} value={this.props.component.scale_min|| ''} placeholder={'Minimum number of replicas when scaling'} onChange={this.props.propsOnChange}
                   instantValidation validations={'isValidInt'} validationError={'Scale Min must be a positive Integer'} errorLabel={errorLabel}/>
          </Form.Field>
          ...
          <Form.Field>
            ...
            <Input name={'scale_max'} value={this.props.component.scale_max|| ''} placeholder={'Maximum number of replicas when scaling'} onChange={this.props.propsOnChange}
                   instantValidation validations={'isValidInt'} validationError={'Scale Max must be a positive Integer'} errorLabel={errorLabel}/>
          </Form.Field>
              ...
              return (
                <div className={'port-input-wrapper'} key={`port-${i}`}>
                  <Input value={port.key}
                         ...
                         validationError={'Host value must be of the form (HOST_IP:)PORT'}
                         errorLabel={errorLabelNoPoint}
                         required
                         validationErrors={{isDefaultRequiredValue: 'Cannot leave port value empty. Delete if not required'}}
                  />
                  ...
                  <Input value={port.value}
                         ...
                         validationError={'Container port must be a positive integer'}
                         errorLabel={errorLabelNoPoint}
                         required
                         validationErrors={{isDefaultRequiredValue: 'Cannot leave port value empty. Delete if not required'}}
                  />
          ...
          <Form.Field>
            <label>Storage <Button type={'button'} className={'add-button'} onClick={this.props.addStore} size={'mini'} icon={'plus'}/></label>
            {this.props.component.storage && this.props.component.storage.map((store, i) => {
              return (
                <div className={'port-input-wrapper'} key={`store-${i}`}>
                  <Search placeholder='Storage name'
                          value={store.name}
                          className={'network-input'}
                          onSearchChange={e => this.props.handleStorageSearchChange(i, e)}
                          onResultSelect={(e, d) => this.props.handleStorageResultSelect(i, e, d)}
                          results={this.props.storageResults}
                          noResultsMessage={'Enter a new storage name to create a new persistent store'}
                  />
                  <Icon name={'arrow right'} className={'port-arrow'}/>
                  <Input value={store.path}
                         className={'port-input'}
                         ...
                         errorLabel={errorLabelNoPoint}
                         required
                         validationErrors={{isDefaultRequiredValue: 'Cannot leave storage path empty. Delete if not required'}}
                  />
                  <Button type={'button'} className={'delete-port'} negative icon={'close'} size={"mini"} onClick={() => this.props.deleteStorage(i)}/>
                </div>
              )
            })}
          </Form.Field>
