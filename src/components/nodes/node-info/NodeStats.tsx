...
function timeSince(unixTime: number) {
  ...
  if (secondsPast < 3600) {
    return `${secondsPast / 60} minutes ago`;
  }
    ...
    <React.Fragment>
      ...
      <Container textAlign={'center'}>
        <div className={'main-stats'}>
          <Statistic>
            <Statistic.Value>{nodeStats.stats[nodeStats.stats.length - 1].cpus}</Statistic.Value>
            <Statistic.Label>CPU core{nodeStats.stats[nodeStats.stats.length - 1].cpus !== 1 ? 's' : ''}</Statistic.Label>
          </Statistic>

          <Statistic>
            <Statistic.Value>{sizeOf(nodeStats.stats[nodeStats.stats.length - 1].total_mem)}</Statistic.Value>
            <Statistic.Label>of Memory</Statistic.Label>
          </Statistic>

          <Statistic>
            <Statistic.Value>{nodeStats.stats[nodeStats.stats.length - 1].num_containers}</Statistic.Value>
            <Statistic.Label>Running Ringleadr
              Container{nodeStats.stats[nodeStats.stats.length - 1].num_containers !== 1 ? 's' : ''}</Statistic.Label>
          </Statistic>
        </div>
      </Container>

      <p>Last recorded: {timeSince(nodeStats.stats[nodeStats.stats.length - 1].timestamp)}</p>
