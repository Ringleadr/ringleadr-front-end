...
import { deleteApp, updateApp } from "../../../api/api";
...
class InfoDisplay extends Component {
  ...
  handleDelete() {
    deleteApp(this.props.app.name).then((resp) => {
      if (resp.ok) {
  ...
  handleUpdate() {
    if (this.state.validJSON) {
      ...
      updateApp(JSON.stringify(updatedApp)).then((resp) => {
        if (resp.ok) {
          this.setState({ showSuccess: true });
          setTimeout(() => {
            this.setState({ redirect: true });
    ...
    return (
      <React.Fragment>
        ...
        <Message negative size="big" floating hidden={!this.state.showFailure}>
          <Message.Header>
            Something went wrong. Response from server:
          </Message.Header>
        ...
        <Grid centered columns={1} padded>
          <Button onClick={this.handleUpdate} disabled={!this.state.validJSON}>
            Update
          </Button>
